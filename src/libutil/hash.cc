    ...
    : Hash(algo)
{
    ...
    if (isSRI) {
        ...
        if (rest.size() == base16::encodedLength(hashSize)) {
            ...
            formatName = "base16";
        } else if (rest.size() == BaseNix32::encodedLength(hashSize)) {
            decode = BaseNix32::decode;
            formatName = "nix32 (Nix's base32 variation)";
        } else if (rest.size() == base64::encodedLength(hashSize)) {
            decode = base64::decode;
            formatName = "base64";
        } else
    ...
    if (d.size() != hashSize)
        throw BadHash("invalid %s hash '%s'", isSRI ? "SRI" : "base64", rest);
    assert(hashSize);
